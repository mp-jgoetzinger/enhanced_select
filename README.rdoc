...
* config.gem "aeonscope-enhanced_select", :lib => "enhanced_select", :source => "http://gems.github.com"
...
While the above is a simple example, lets look at a more interesting case. Using the example above, lets say we want each school
to have a CSS class associated with it so that we might be able to attach jQuery behaviors to each option (that and/or we want
to style each option too). Here is how things might play out:
...
<b>Model Code</b>
  ...
  class School < ActiveRecord::Base
    ...
    def kind
      [(public? ? "public" : nil), (private? ? "private" : nil)].compact.join(' ')
    end
  end
