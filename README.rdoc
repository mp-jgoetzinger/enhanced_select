...
While the above is a simple example, lets look at a more interesting case. Continuing with the example above, lets say we want each school to have a CSS class associated with it so that we might be able to attach jQuery behaviors to each option (that and/or we want to style each option too). Here is how things might play out:
